...
chrome.runtime.onMessage.addListener(request => {
    ...
    chrome.storage.local.get('tokens', async (lib) => { 
        ...
        const ingVectors = vectorize(lines, ingTokens)
        const methVectors = vectorize(lines, methTokens)
        ...
        for (let i = 0; i < lines.length; i++) {    
            ...
            if (methPrediction[i][0] < methPrediction[i][1] && methVectors[i][9] === 0) {  // avoid lines with ING PATTERN
                ...
                if (!results.ing.includes(lines[i])) { results.all.push(lines[i]) }  // no repeated LINES
            }
        }
...
function vectorize(lines, tokens) {
    ...
    lines.forEach(line => {
        ...
        let score = getTokensScore(words, tokens)
        scores.push(score)
        vectors.push([words.length, verb, score])
    });
    ...
    for (let i in vectors) {
        vectors[i].push(
            ...
            scores[i+2] || 0,
            scores[i+3] || 0,
            lines[i].match(ING_PATTERNS[0]) ? 1 : 0,
            lines[i].match(ING_PATTERNS[1]) ? 1 : 0
        )
    }
